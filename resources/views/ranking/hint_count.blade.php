...
@section('title', '文字の消去回数ランキング - 重ね文字クイズ')

@section('content')

<h1 class="text-2xl font-bold text-center text-blue-700 mb-6">文字の消去回数ランキング</h1>
  ...
  <table class="w-full text-sm text-gray-800">
    <thead class="bg-slate-100 text-left border-b border-gray-300">
      <tr>
        ...
        <th class="px-4 py-3 font-semibold text-gray-700">消去回数</th>
      </tr>
    </thead>
    <tbody>
    @php
    ...
    @foreach($words as $index => $word)
      @php
        ...
        if ($word->wordStatistics->play_count !== $prevCount) {
            $displayRank = $rank;
            $prevCount = $word->wordStatistics->hint_count;
        }
      @endphp
      <tr class="border-t">
